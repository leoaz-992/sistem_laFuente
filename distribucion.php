...
if (isset($_GET['id_pedido'])) {
  ...
  $sqlUpdate = "UPDATE pedidos SET fecha_entrega = CURRENT_TIMESTAMP(), estado_pedido_id = '1', statusPago_id = '2' WHERE id_pedido = ?";
  $stmt = mysqli_prepare($connection, $sqlUpdate);
  mysqli_stmt_bind_param($stmt, "i", $id_recibido);
  if (mysqli_stmt_execute($stmt)) {
      redirigirADistribucion();
  }
}

if (isset($_GET['id_pedido_quitar'])) {
  $id_recibido = $_GET['id_pedido_quitar'];
  $sqlDeleteDetalle = "DELETE FROM detallespedidos WHERE pedido_id = ?";
  $sqlDelete = "DELETE FROM pedidos WHERE id_pedido = ?";
  
  $stmtDetalle = mysqli_prepare($connection, $sqlDeleteDetalle);
  $stmtDelete = mysqli_prepare($connection, $sqlDelete);

  mysqli_stmt_bind_param($stmtDetalle, "i", $id_recibido);
  mysqli_stmt_bind_param($stmtDelete, "i", $id_recibido);

  if (mysqli_stmt_execute($stmtDetalle) && mysqli_stmt_execute($stmtDelete)) {
      redirigirADistribucion();
  }
}
      ...
      <tbody>
          ...
          if (mysqli_num_rows($resultado) > 0) {
              while ($pedido = mysqli_fetch_array($resultado)) {
                      ...
                      echo "<td>{$pedido["altura"]}</td>"; // es el numero de la calle.
                      echo "<td>{$pedido["barrio"]}</td>";
                      echo "<td>{$pedido["metodo_de_pago"]}</td>";
                      echo "<td>{$pedido["estado_de_pago"]}</td>";
                      echo "<td>" . ($pedido["dia_entrega"] == null ? "pendiente" : $pedido["dia_entrega"]) . "</td>";
                      echo "<td>
                              <a class='btn btn-info btn-sm m-1' href='distribucion.php?id_pedido={$pedido['idPedido']}'>pedido entregado</a>
                               <a class='btn btn-danger btn-sm m-1' href='distribucion.php?id_pedido_quitar={$pedido['idPedido']}'>quitar pedido</a>
                              <a class='btn btn-warning btn-sm m-1' href='detallePedido.php?id_pedido={$pedido['idPedido']}'>detalle de pedido</a>
                            </td>";
                      echo "</tr>";
                  }
              }
...
?>
Estas sugerencias ayudarán a que tu código sea 